...
parser.add_argument('-o','-U','--output', default = "out_test.avi",
                    help='Output video file, default as "out_test.avi"')     #输入文件
...
parser.add_argument('-e','-E','--End', type = int, default = 1,
                    help='End at X second, default at 1s')     #输入文件
...
## How to use it
```bash
## print the video from 0s to 1s to out_test.avi
Video_slice.py -i bug.avi

## print the video from 10s to 20s to sliced.avi file
Video_slice.py -i bug.avi -s 10 -e 20 -o sliced.avi
```
